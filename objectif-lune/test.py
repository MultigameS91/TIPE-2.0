...
def descente_gradient_adam(x0, y0, xn, yn, v0, U0, nombre_points, epsilon, affichage=True):
    def temps_from_points(vars):
        bezier = construire_bezier(x0, y0, xn, yn, vars)
        return calcul_temps_courbe(bezier, v0)

    geodesique = None
    m = np.zeros_like(U0)  # Moment
    v = np.zeros_like(U0)  # Accumulation des gradients au carré
    beta1 = 0.9
    beta2 = 0.999
    epsilon_adam = 1e-8
    ...
    while k < max_iterations:
        ...
        # Adam update
        m = beta1 * m + (1 - beta1) * Gradk
        v = beta2 * v + (1 - beta2) * (Gradk ** 2)
        m_hat = m / (1 - beta1 ** k)
        v_hat = v / (1 - beta2 ** k)
        Uk -= lr * m_hat / (np.sqrt(v_hat) + epsilon_adam)

        print(f"Iteration {k}: T = {temps_from_points(Uk):.6f}, |grad| = {np.linalg.norm(Gradk):.6f}")
        if np.linalg.norm(Gradk) < epsilon:
            break
        k += 1
...
def monte_carlo(x0, y0, xn, yn, v0, nombre_points_max, epsilon, affichage):
    ...
    for n in range(2, nombre_points_max + 1):
        for i in range(50):
            ...
            geod, duree = descente_gradient_adam(x0, y0, xn, yn, v0, U0, n, epsilon, affichage=False)
            if duree < duree_min:
                geodesique = geod
                duree_min = duree
...
monte_carlo(Terre.pos[0], Terre.pos[1], Mars.pos[0], Mars.pos[1], array([0, 12e3]), 8, 1, affichage=True)
