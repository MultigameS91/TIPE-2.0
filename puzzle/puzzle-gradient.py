...
carte_puzzle = img.imread('C:\\Users\\yopla\\Desktop\\TIPE 2.0\\puzzle\\puzzle2.png') 
...
for x in range(n):
    for y in range(m):
        ...
        fils=avoir_fils(x,y)
        aretes[(x,y)]=fils
        sommets[(x,y)]= poids(carte_puzzle[x,y])
...
indice_max=1
indice_min=0
niveau_de_gris=hauteur*(161*(indice_max-indice_min))
...
def djikstra(G,depart,arrivee):
    ...
    while sommet != arrivee:
        ...
        for v in voisins:
            if not deja_vus[v]:
                deja_vus[v]=True
                delta=abs(G.sommets[sommet]-G.sommets[v])
                dist=np.sqrt((delta*niveau_de_gris)**2+1)+distances[sommet]+distance(arrivee[0],arrivee[1],v[0],v[1])*0.1
                if endiago(v[0],v[1],sommet[0],sommet[1]):
                    dist=np.sqrt((delta*niveau_de_gris)**2+2)+distances[sommet]+distance(arrivee[0],arrivee[1],v[0],v[1])*0.1
                if distances[v]>dist:
                    distances[v]=dist
